# By Rhekar at https://github.com/Rhekar/CCCasterWebsite/blob/master/comboparser.py
...
class ComboTransformer(Transformer):
    ...
    def parseSeq(self, fname):
        ...
        with open(f"/seqlists/{fname}.csv", "r") as f:
            f.readline()
            for line in f:
                l = line.strip().split(",")
                name = l[0]
    ...
    def hit_input(self, items):
        ...
        for item in items:
            ...
            if item.type == "DIRECTION":
                ...
                dispString += item
            if item.type == "JUMP" or item.type == "BUTTON" or item.type == "DIRECTION":
                if item[0] == "[" or item[0] == "{":
        ...
        for item in items:
            ...
            if item.type == "BUTTON":
                ...
                if firstInput:
                    firstInput = False
                    dispString = (
                        delayString + dispPrefString + toArrow(direction) + item.upper()
                    )
                    delayString = ""
                    ...
                    dispString = (
                        "Add. " + dispPrefString + toArrow(direction) + item.upper()
                    )
                    if direction == "5":
                        direction = ""
    ...
    def MOVEMENT(self, items):
        ...
        return (dispString, self.seqDict[moveString], "0", moveString)
